...
SELECT
    ...
    (SELECT sum(readmissions_30j) FROM eds_gold_pilotage.kpi_readmissions_30j) AS nb_readmissions_30j,
    (SELECT round(100.0 * sum(readmissions_30j) / sum(sorties_eligibles), 1)
     FROM eds_gold_pilotage.kpi_readmissions_30j)                           AS taux_readmission_pct,
