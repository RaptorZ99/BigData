...
| `kpi_readmissions_30j` | service × jour de sortie | Dénominateur : sorties vivantes (`discharge_ts` non NULL, `discharge_mode != 'deces'`). Numérateur : celles suivies d'une admission du même patient dans [0 ; 30] jours. Fenêtre : `leadInFrame(admission_ts) OVER (PARTITION BY patient_pseudo ORDER BY admission_ts ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING)`. Attendu global (sur fact_sejour nettoyé) : **356 réadmissions / 11 678 sorties éligibles ≈ 3,0 %**. |
...
| **P5 Gold + RBAC** | 30_gold, users/GRANTs | KPI cohérents (réadmissions 356/11 678 ≈ 3,0 % ; alertes 3 053) ; `chu_recherche` ne lit pas pilotage (testé) ; aucune cohorte < 5 diffusée (4 cellules supprimées sur la vue fine par département). |
